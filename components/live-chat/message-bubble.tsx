  ...
  return (
    <div className={`group relative flex ${isMine ? 'justify-end' : 'justify-start'} px-2 py-1`}>
      ...
      >
        ...
        <div className={isNakedEmoji ? 'relative bg-transparent px-1 py-1' : `relative rounded-2xl px-3.5 py-2 ${isAiRole ? 'bg-gradient-to-br from-teal-600 to-cyan-600 text-white' : isMine ? 'bg-gradient-to-br from-[var(--lc-mine-from,#2563eb)] to-[var(--lc-mine-to,#06b6d4)] text-[var(--lc-mine-text,#ffffff)]' : 'bg-[var(--lc-their-bg,var(--muted))] text-[var(--lc-their-text,var(--foreground))]'}`}>
          {replyToMessage && (
            <button
